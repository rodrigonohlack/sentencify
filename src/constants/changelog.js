...
export const CHANGELOG = [
  { version: '1.43.01', feature: 'fix(gemini-cache): cache não disparava em NENHUMA análise do app principal (petição inicial, extração de tópicos, etc) com qualquer modo de OCR (pdf.js, pdf-puro, claude-vision, tesseract). Bug crítico: o split foi desenhado pensando em chat multi-turn ("última mensagem = volátil"), mas o Sentencify monta análises como UMA mensagem do user com vários parts (contexto + prompt) — caía 100% em volátil em bloco, threshold nem era avaliado contra o conteúdo dos PDFs. Fix em duas frentes: (1) splitting a nível de PART quando há 1 só mensagem do user com >= 2 parts: parts iniciais com docs (inlineData/text longo/marker [PROVA:/CONTESTAÇÃO/etc]) → cache; última part (a pergunta/instrução curta) → volátil; (2) detectar inlineData/inline_data (PDFs em base64 do modo pdf-puro/claude-vision) como part estável — antes só fileData/file_data (mídia da File API) era reconhecido. Cobre todos os modos: pdf.js (texto longo), pdf-puro (inlineData), claude-vision (inlineData), tesseract (texto longo). +6 testes incluindo single-shot com PDF base64, marker em texto curto, fallback sem stable parts.' },
  { version: '1.43.00', feature: 'feat(gemini): context caching nativo em toda chamada Gemini + suporte a áudio/vídeo na aba de Provas. (1) Cache automático: GeminiCacheService usa hash SHA-256 de (model + system + documentos estáveis) para reusar CachedContent — TTL 1h, threshold 4k tokens, idempotente, retry transparente em CACHE_NOT_FOUND. Frontend separa "estável" (system + parts com fileData + textos longos/com marcadores) do "volátil" (a pergunta atual). Métricas cache_read/cache_creation já caem no painel via cachedContentTokenCount. (2) Aba de Provas aceita upload de áudio (MP3/WAV/AAC/OGG/FLAC) e vídeo (MP4/MOV/WEBM) quando provider==="gemini" && autenticado. Upload em STREAMING pass-through pelo backend (Readable.toWeb + octet-stream) → Gemini File API: nada gravado em disco, RAM ~64KB constante. Após ACTIVE, CachedContent é criado automaticamente. Player HTML5 com timestamps MM:SS clicáveis na resposta. Atalhos: transcrição com timestamps, síntese de ata, contradições, confissões, pontos por pedido. (3) Migration 020: gemini_caches + gemini_media_files (só metadados, ~500B/row). Cron horário (GeminiPruneService) limpa expirados e força DELETE no Google. (4) Endpoints novos: POST/PATCH/DELETE /api/gemini/cache, /api/gemini/media/{upload,status,cache,delete}. /generate e /stream aceitam cachedContent. (5) HARDENING pós-review: (a) IDOR fix em markUsed — agora valida userId; (b) recriação automática de cache de mídia em CACHE_NOT_FOUND via /media/:id/cache + retry; (c) tracking de hit_count para mídia via lookup-by-name (antes a métrica nunca incrementava); (d) prune de mídia usa _internalDeleteCache (antes deletava nada por causa de userId=null); (e) race condition na criação concorrente: best-effort delete do CachedContent perdedor; (f) validação de headers de upload (filename pattern + MIME whitelist + path traversal); (g) polling de status detecta 401 (antes spinava até 10min); (h) objectURL revogado em failure (antes vazava 200-500MB); (i) shouldCacheRequest exige stableContents>0 (antes gerava 400 silencioso a cada chamada com system longo); (j) cachedContent não duplicado em request body; (k) temas claro/escuro em MediaProofCard, ProofsTab e atalhos de mídia; (l) engines.node ≥20 em package.json. +4 testes (IDOR, lookup-by-name, sem-userId, system-só-sem-stable).' },
