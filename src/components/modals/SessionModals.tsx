...
export const RestoreSessionModal = React.memo(({ isOpen, onClose, sessionLastSaved, onRestoreSession, onStartNew }: RestoreSessionModalProps) => (
  <BaseModal isOpen={isOpen} onClose={onClose} title="Sessão Anterior Encontrada" icon={<Save />} iconColor="blue" size="sm" preventClose
    footer={<><button onClick={onRestoreSession} className={`${CSS.btnBlue} flex items-center justify-center gap-1.5`}><CheckCircle2 className="w-4 h-4" aria-hidden="true" /> Continuar Sessão</button><button onClick={onStartNew} className={`${CSS.btnRed} flex items-center justify-center gap-1.5`}><Trash2 className="w-4 h-4" aria-hidden="true" /> Começar do Zero</button></>}>
