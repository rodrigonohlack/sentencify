 ...
 * @version 1.39.09
 *
 * Movido de src/apps/prova-oral/components/StreamingModal.tsx
 * Agora compartilhado entre Sentencify principal e Prova Oral
 */
...
/** Tipos de operação para personalização do modal */
...
/** Configurações de exibição por tipo de operação */
const OPERATION_CONFIG: Record<StreamingOperationType, { title: string; subtitle: string }> = {
  'report': { title: 'Gerando Mini-Relatório', subtitle: 'Analisando documentos e gerando relatório' },
  'proof': { title: 'Analisando Prova', subtitle: 'Processando análise da prova' },
  'dispositivo': { title: 'Gerando Dispositivo', subtitle: 'Redigindo dispositivo da sentença' },
  'chat': { title: 'Processando Resposta', subtitle: 'Gerando resposta do assistente' },
  'prova-oral': { title: 'Analisando Prova Oral', subtitle: 'Processando transcrição da audiência' },
  'generic': { title: 'Processando', subtitle: 'Aguarde o processamento' }
};
...
export interface StreamingModalProps {
  ...
  providerName?: string;
  /** Tipo de operação (para personalização do título) */
  operationType?: StreamingOperationType;
  /** Título customizado (sobrescreve operationType) */
  customTitle?: string;
  /** Subtítulo customizado (sobrescreve operationType) */
...
export const StreamingModal: React.FC<StreamingModalProps> = ({
  ...
  // Resolver título e subtítulo
  const config = OPERATION_CONFIG[operationType];
  const title = customTitle || config.title;
  const subtitle = customSubtitle || `${config.subtitle} via ${providerName}`;
      ...
      {!isComplete && (
        <div className="mt-4 p-3 rounded-lg bg-amber-500/10 border border-amber-500/20">
          <p className="text-xs text-amber-600 dark:text-amber-400 text-center">
            Aguarde a conclusão do processamento. Fechar esta janela pode interromper a operação.
          </p>
        </div>
      )}
