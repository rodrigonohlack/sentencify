...
import type { GeminiThinkingLevel, OpenAIReasoningLevel, TokenMetrics, ClaudeCliEffort, CodexCliReasoning } from '../../types';
...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose }) => {
  ...
  const codexCliReasoning = useAIStore((s) => s.aiSettings.codexCliReasoning);
          ...
          {provider === 'claude' && (
            <div className="space-y-3">
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                />
                <span className="text-sm text-slate-700 dark:text-slate-200">Extended Thinking</span>
              </label>
              {useExtendedThinking && (
                <div>
                  ...
                  <input
                    ...
                    min="1000"
                    max="100000"
                    className="w-full px-3 py-2 text-sm border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500"
                  />
                </div>
              )}
            </div>
          ...
          {provider === 'codex-cli' && (
            <div className="space-y-3">
              <div>
                ...
                <select
                  value={codexCliReasoning || 'medium'}
                  onChange={(e) => setAiSettings({ ...useAIStore.getState().aiSettings, codexCliReasoning: e.target.value as CodexCliReasoning })}
                  className="w-full px-3 py-2 text-sm border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500"
                >
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                  Esforço de raciocínio do Codex local (model_reasoning_effort).
                </p>
              </div>
            </div>
          )}
            ...
            <div className="mt-4 bg-slate-50 dark:bg-slate-700/50 rounded-lg p-3">
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                <div className="flex justify-between">
                  <span className="text-blue-600 dark:text-blue-400">Sonnet 4</span>
