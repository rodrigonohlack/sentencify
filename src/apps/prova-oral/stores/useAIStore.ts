/**
 * @file useAIStore.ts
 * @description Store Zustand para configurações de IA do app Prova Oral
 */
  ...
  persist(
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        if (state) {
          try {
            // 1. Tentar ler keys do Prova Oral
            let rawKeys: Record<string, string> | null = null;
            const provaOralKeys = localStorage.getItem('prova-oral-api-keys');
            if (provaOralKeys) {
              rawKeys = JSON.parse(provaOralKeys);
            }

            // 2. Fallback: ler do Analisador se Prova Oral não tem keys configuradas
            ...
            // 3. Fallback: ler do Sentencify se ainda não tem keys
            if (!rawKeys || !Object.values(rawKeys).some(k => k)) {
            ...
            console.warn('[ProvaOral AIStore] Erro ao restaurar apiKeys:', err);
          }
        }
      }
    }
...
export const persistApiKeys = (apiKeys: AISettings['apiKeys']) => {
  encryptApiKeys(apiKeys as unknown as Record<string, string>).then(encrypted => {
    try {
      localStorage.setItem('prova-oral-api-keys', JSON.stringify(encrypted));
    } catch (err) {
      console.warn('[ProvaOral AIStore] Erro ao salvar apiKeys:', err);
    }
  }).catch(err => {
    console.warn('[ProvaOral AIStore] Erro ao encriptar apiKeys:', err);
  });
};
