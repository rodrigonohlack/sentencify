...
export default function ProviderSelector({ value, onChange }: ProviderSelectorProps) {
  return (
    <div>
      <label className="text-xs font-semibold text-[#7c7caa] dark:text-gray-400 uppercase tracking-wider block mb-3">
        ...
        {providers.map((p) => (
          <button
            ...
            className={`glass-card p-4 text-left transition-all ${
              value === p.id
                ? 'ring-2 ring-indigo-500 bg-white/65 dark:bg-white/10 shadow-lg shadow-indigo-500/10'
                : 'hover:bg-white/60 dark:hover:bg-white/10'
            }`}
          >
            <div className="flex items-center gap-2 mb-2">
              <Sparkles className={`w-4 h-4 ${value === p.id ? 'text-indigo-500' : 'text-[#7c7caa]'}`} />
              <span className="text-sm font-bold text-[#1e1b4b] dark:text-gray-100">{p.name}</span>
            </div>
            <p className="text-xs text-[#7c7caa] dark:text-gray-400">{p.description}</p>
          </button>
        ))}
      </div>
    </div>
