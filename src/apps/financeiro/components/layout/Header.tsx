  ...
  return (
    <header className="flex items-center justify-between mb-6 px-1">
      <div>
        ...
        {subtitle && <p className="text-sm text-[#7c7caa] font-normal mt-0.5">{subtitle}</p>}
      </div>

      <div className="flex items-center gap-3">
        {reminders.length > 0 && (
          <div className="relative">
            <button className="p-2.5 bg-white/55 backdrop-blur-lg border border-white/70 rounded-[14px] hover:bg-white/65 transition-all">
        ...
        {month && (
          <div className="flex items-center gap-1">
            <button
              onClick={onPrevMonth}
              className="p-2 bg-white/55 backdrop-blur-lg border border-white/70 rounded-xl hover:bg-white/65 transition-all"
            >
              <ChevronLeft className="w-4 h-4 text-indigo-500" />
            </button>
            <button className="flex items-center gap-2.5 bg-white/55 backdrop-blur-lg border border-white/70 rounded-[14px] px-5 py-2.5 font-semibold text-sm text-[#1e1b4b] hover:bg-white/65 transition-all">
              <Calendar className="w-[18px] h-[18px] text-indigo-500" />
              {formatMonthLabel(month)}
              <ChevronDown className="w-3.5 h-3.5 text-[#7c7caa]" />
            </button>
            <button
              onClick={onNextMonth}
              className="p-2 bg-white/55 backdrop-blur-lg border border-white/70 rounded-xl hover:bg-white/65 transition-all"
