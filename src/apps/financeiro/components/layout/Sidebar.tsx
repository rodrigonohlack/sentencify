...
export default function Sidebar() {
  ...
  return (
    <aside className="fixed top-4 left-4 bottom-4 w-[260px] bg-[rgba(255,255,255,0.25)] backdrop-blur-[24px] border border-white/70 rounded-[24px] flex flex-col z-[100] overflow-hidden">
      {/* Brand */}
      <div className="px-6 pt-7 pb-5 flex items-center justify-between">
        ...
        <AppSwitcher
          ...
          className="flex items-center gap-1 px-2 py-1.5 text-sm text-[#7c7caa] hover:text-indigo-600 transition-colors rounded-lg hover:bg-white/40"
        >
          ...
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-[14px] text-sm font-medium mb-1 transition-all duration-250 ${
              currentPage === page
                ...
                : 'text-[#7c7caa] hover:bg-white/40 hover:text-[#1e1b4b]'
            }`}
          >
      ...
      <div className="px-5 py-4 border-t border-white/30">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-[13px] font-semibold text-[#1e1b4b] truncate">
              {userEmail?.split('@')[0]}
            </div>
            <div className="text-[11px] text-[#7c7caa] truncate">{userEmail}</div>
          </div>
          <button
            onClick={logout}
            ...
            title="Sair"
          >
            <LogOut className="w-4 h-4 text-[#7c7caa]" />
          </button>
        </div>
      </div>
    </aside>
