  ...
  return (
    <div className={`glass-card flex items-center justify-between ${!item.is_active ? 'opacity-60' : ''}`}>
      <div className="flex items-center gap-4">
        ...
        <div>
          <div className="text-sm font-semibold text-[#1e1b4b]">{item.description}</div>
      ...
      <div className="flex items-center gap-4">
        <span className="text-lg font-bold text-[#1e1b4b] tabular-nums">{formatBRL(item.value_brl)}</span>
        <div className="flex items-center gap-1">
          <button onClick={() => onToggle(item.id)} className="p-2 rounded-xl hover:bg-white/50 transition-colors" title={item.is_active ? 'Pausar' : 'Ativar'}>
            {item.is_active ? <Pause className="w-4 h-4 text-amber-500" /> : <Play className="w-4 h-4 text-emerald-500" />}
          </button>
          <button onClick={() => onEdit(item.id)} className="p-2 rounded-xl hover:bg-white/50 transition-colors">
            <Pencil className="w-4 h-4 text-[#7c7caa]" />
          </button>
          <button onClick={() => onDelete(item.id)} className="p-2 rounded-xl hover:bg-red-50 transition-colors">
