...
const docxPath = path.join(__dirname, 'OJ SDI1 TST.docx');
const outputPath = path.join(__dirname, 'oj-sdi1-tst.json');
...
async function main() {
  ...
  // Dividir por "OJ-SDI1-" no início
  const blocos = content.split(/(?=OJ-SDI1-\d+\t)/gi).filter(b => b.trim());
  ...
  for (const bloco of blocos) {
    try {
      ...
      const headerMatch = texto.match(/OJ-SDI1-(\d+)\t/i);
      if (!headerMatch) continue;

      const numero = parseInt(headerMatch[1]);
      const linhas = texto.split(/\n+/).map(l => l.trim()).filter(l => l);

      // Primeira linha: "OJ-SDI1-X\tTÍTULO (status) - DJ ..."
      const primeiraLinha = linhas[0] || '';

      // Extrair título (após o tab, antes do parêntese ou " - ")
...
function extrairKeywords(texto) {
  ...
  const termos = [
    ...
    'dano moral', 'assédio', 'responsabilidade'
  ];
