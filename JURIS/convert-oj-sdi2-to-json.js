...
const docxPath = path.join(__dirname, 'OJ SDI2 TST.docx');
const outputPath = path.join(__dirname, 'oj-sdi2-tst.json');
...
async function main() {
  ...
  // Dividir por "OJ-SDI2-" no início
  const blocos = content.split(/(?=OJ-SDI2-\d+\t)/gi).filter(b => b.trim());
  ...
  for (const bloco of blocos) {
    try {
      ...
      const headerMatch = texto.match(/OJ-SDI2-(\d+)\t/i);
...
function extrairKeywords(texto) {
  ...
  const termos = [
    'ação rescisória', 'mandado de segurança', 'habeas corpus', 'tutela',
    'competência', 'execução', 'recurso', 'embargos', 'agravo',
    'prescrição', 'decadência', 'coisa julgada', 'preclusão',
    ...
    'sindicato', 'contribuição', 'negociação coletiva',
    'honorários', 'custas', 'depósito recursal', 'dano moral'
  ];
