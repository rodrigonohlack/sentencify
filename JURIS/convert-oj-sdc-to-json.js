...
const docxPath = path.join(__dirname, 'OJ SDC TST.docx');
const outputPath = path.join(__dirname, 'oj-sdc-tst.json');
...
async function main() {
  ...
  // Dividir por "OJ-SDC-" no início
  const blocos = content.split(/(?=OJ-SDC-\d+\t)/gi).filter(b => b.trim());
  ...
  for (const bloco of blocos) {
    try {
      ...
      const headerMatch = texto.match(/OJ-SDC-(\d+)\t/i);
      ...
      // Extrair título (capitalizar primeira letra de cada palavra importante)
...
function extrairKeywords(texto) {
  ...
  const termos = [
    'dissídio coletivo', 'greve', 'acordo coletivo', 'convenção coletiva',
    'sindicato', 'categoria', 'base territorial', 'representação',
    'negociação coletiva', 'cláusula', 'reajuste', 'piso salarial',
    'competência', 'legitimidade', 'pessoa jurídica', 'direito público',
    'multa', 'contribuição', 'assembléia', 'quórum',
    'abusividade', 'lockout', 'substituição processual',
    'ultratividade', 'vigência', 'norma coletiva',
    'empregado', 'empregador', 'trabalhista', 'CLT'
  ];
